...
def train(config, workdir):
  ...
  # Build data iterators for later stage
  old_batch_size = config.training.batch_size
  config.training.batch_size = 128
  train_ds_later, eval_ds_later, _ = datasets.get_dataset(config,
                                              additional_dim=config.training.n_jitted_steps,
                                              uniform_dequantization=config.data.uniform_dequantization)
  train_iter_later = iter(train_ds_later)  # pytype: disable=wrong-arg-types
  eval_iter_later = iter(eval_ds_later)  # pytype: disable=wrong-arg-types
  ...
  for step in range(initial_step, num_train_steps + 1, config.training.n_jitted_steps):
    ...
    if step <= 10000:
      ...
      batch = jax.tree_map(lambda x: scaler(x._numpy()), next(train_iter_later))  # pylint: disable=protected-access
    rng, *next_rng = jax.random.split(rng, num=jax.local_device_count() + 1)
    next_rng = jnp.asarray(next_rng)
    # Execute one training step
    (_, pstate), (ploss, plosses_1, plosses_2, plosses_3, ploss_t) = p_train_step((next_rng, pstate), batch)
    loss = flax.jax_utils.unreplicate(ploss).mean()
    losses_1 = flax.jax_utils.unreplicate(plosses_1).mean()
    losses_2 = flax.jax_utils.unreplicate(plosses_2).mean()
    losses_3 = flax.jax_utils.unreplicate(plosses_3).mean()
    loss_t = flax.jax_utils.unreplicate(ploss_t).mean()
    ...
    if jax.host_id() == 0 and step % config.training.log_freq == 0:
      ...
      writer.scalar("training_loss_1", losses_1, step)
      writer.scalar("training_loss_2", losses_2, step)
      writer.scalar("training_loss_3", losses_3, step)
      writer.scalar("training_loss_t", loss_t, step)
    ...
    if step % config.training.eval_freq == 0:
      ...
      rng, *next_rng = jax.random.split(rng, num=jax.local_device_count() + 1)
      next_rng = jnp.asarray(next_rng)
      (_, _), (peval_loss, losses_1, losses_2, losses_3, loss_t) = p_eval_step((next_rng, pstate), eval_batch)
      eval_loss = flax.jax_utils.unreplicate(losses_1).mean()
      if jax.host_id() == 0:
  ...
  for ckpt in range(begin_ckpt, config.eval.end_ckpt + 1):
    ...
    if config.eval.enable_loss:
      ...
      for i, batch in enumerate(eval_iter):
        ...
        rng, *next_rng = jax.random.split(rng, num=jax.local_device_count() + 1)
        next_rng = jnp.asarray(next_rng)
        (_, _), (p_eval_loss, losses_1, losses_2, losses_3, loss_t) = p_eval_step((next_rng, pstate), eval_batch)
