...
class VPSDE(SDE):
  ...
  def mar(self, x, t):
    log_mean_coeff = -0.25 * t ** 2 * (self.beta_1 - self.beta_0) - 0.5 * t * self.beta_0
    alpha = jnp.exp(log_mean_coeff)
    beta = jnp.sqrt(1 - jnp.exp(2. * log_mean_coeff))
    return alpha, beta, alpha / beta

  def d_drift(self, x, t):
    return (self.beta_0 + t * (self.beta_1 - self.beta_0)) * -0.5
...
class subVPSDE(SDE):
  ...
  def mar(self, x, t):
    log_mean_coeff = -0.25 * t ** 2 * (self.beta_1 - self.beta_0) - 0.5 * t * self.beta_0
    alpha = jnp.exp(log_mean_coeff)
    beta = 1 - jnp.exp(2. * log_mean_coeff)
    return alpha, beta, alpha / beta
...
class VESDE(SDE):
  ...
  def mar(self, x, t):
    alpha = 1
    beta = self.sigma_min * (self.sigma_max / self.sigma_min) ** t
    return alpha, beta, alpha / beta
